...
public class MultiPage<P extends JavaPlugin> extends AbstractMenu<P> {

    private final String title;
    private final int size, prevPageSlot, nextPageSlot;

    @Getter private final Collection<? extends Button<P>> buttons;

    private PageNode first;

    @Setter private ItemStack spaceFiller;
    @Setter private ItemStack prevButton;
    @Setter private ItemStack nextButton;
    @Setter private ItemStack emptyIndicator;

    public MultiPage(final P plugin, final String title, final int rows, final Collection<? extends Button<P>> buttons) {
        super(plugin);
    ...
    private ItemStack getSpaceFiller() {
        return spaceFiller != null ? spaceFiller : new ItemStack(Material.WHITE_STAINED_GLASS_PANE);
    }
    ...
    private class PageNode {
        ...
        void setTitle(final String title) {
            InventoryUtils.setTitle(inventory, title);
        }
        ...
        void resetBottom() {
            InventoryUtils.run(prevPageSlot, nextPageSlot + 1, slot -> inventory.setItem(slot, getSpaceFiller()));
        }
