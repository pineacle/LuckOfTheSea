...
public abstract class AbstractMenu<P extends JavaPlugin> {

    protected final P plugin;

    private final Map<Inventory, Map<Integer, Button<P>>> buttons = new HashMap<>();

    protected AbstractMenu(P plugin) {
        this.plugin = plugin;
    }
    ...
    public void update(final Player player, final Inventory inventory, final Button<P> button) {
        ...
        cached.entrySet().stream().filter(entry -> entry.getValue().equals(button)).findFirst().ifPresent(entry -> inventory.setItem(entry.getKey(), button.getDisplay()));
    }
